...
**Survivorship caveat (R3/R4):** every backfill row is a unit TAV **bought and sold**. There are no `no_sale` / pass-on / counterfactual rows — those are `future-only` (logging starts day one per [`TECHNICAL-SPEC.md`](../../../../docs/07-buybox/TECHNICAL-SPEC.md) §1.5). The backfill is a bought-unit dataset by construction, consistent with the v1/v2 scope.
...
The backfill columns map onto the existing `tav.purchase_outcomes` baseline (per [`audits/06-field-completeness-audit.md`](../06-field-completeness-audit.md) §3) plus the [`TECHNICAL-SPEC.md`](../../../../docs/07-buybox/TECHNICAL-SPEC.md) §1.1 additions. Measured fill rate is over all 57,228 rows.
