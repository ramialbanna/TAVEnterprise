-- Item 62 / item 60 Phase B — persist seller listing description for detail UI + re-scrapes.

ALTER TABLE tav.normalized_listings
  ADD COLUMN IF NOT EXISTS description text;

COMMENT ON COLUMN tav.normalized_listings.description IS
  'Seller free-text from Apify extraListingData.description (marketplace listing mirror).';

-- Replace prior 17-arg RPC (Postgres treats different arg lists as overloads).
DROP FUNCTION IF EXISTS tav.upsert_normalized_listing(
  text, uuid, text, text, text, integer, integer, smallint, text, text, text, text, text, timestamptz, text, text, text[], timestamptz, uuid
);
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO tav.normalized_listings (
      ...
      scraped_at, seller_name, seller_url, images, posted_at, description,
      ...
      p_scraped_at, p_seller_name, p_seller_url, p_images, p_posted_at, p_description,
