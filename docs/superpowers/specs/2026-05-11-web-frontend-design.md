 ...
 Browser (TAV staff)  ─────►  Next.js App Router app  ─────►  Cloudflare Worker `tav-aip`
   ...
   - never sees secrets         - /api/app/[...path] proxy            ├─► Supabase Postgres (service role, in-Worker only)
                                ...
                                - server-only env: APP_API_SECRET,    └─► tav-intelligence-worker  (Service Binding / public fetch)
                                  AUTH_SECRET, AUTH_GOOGLE_*                 - Cox/Manheim MMR  (sandbox-backed in prod for now)
```
...
**Data source (live):** `POST /app/mmr/vin` (body `{vin, year?, mileage?}`). **Cox MMR is sandbox-backed in production** — show a small inline notice on this page mirroring the Admin caveat string so a user doesn't mistake a sandbox value for a live wholesale figure.
...
- **Cox / Manheim status** — derived from `intelWorker` + the standing caveat. **Persistent `CaveatBanner` with the exact string:** *"Cox MMR is currently sandbox-backed in production until Cox enables true production MMR credentials."* Always visible on this page (and mirrored as an inline notice on the MMR Lab page).
- **Cox environment** — explicit label: **Sandbox-backed** (current) vs **Production-enabled** (future). v1 reads "Sandbox-backed" from the caveat (there's no machine-readable flag in `system-status` yet — note that as a tiny API-gap nicety; until then it's a static label tied to the caveat).
...
- **Phase 3 — VIN / MMR Lookup Lab** (`/mmr-lab`). Form + mutation + result panel + spread + heuristic recommendation + raw-payload viewer + preview-VIN "fill example" + the historical-comparison panel (client-side aggregates over `/app/historical-sales`, rest `PendingBackend`) + the sandbox caveat notice. Tests + Playwright VIN-lookup key flow (mocked `/api/app/mmr/vin`).
...
- **E2E (Playwright) — v1 depth (ii):**
  ...
  - Admin/Integrations: the exact Cox-sandbox caveat string is present.
