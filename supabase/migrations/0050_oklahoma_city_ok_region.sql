-- =============================================================================
-- Migration 0050 — Add oklahoma_city_ok region (Phase 8: tav-ok soak)
--
-- Apify task tav-ok (Xpq656NgueqfXDHvU) scrapes Oklahoma City, OK. Extends
-- region CHECK constraints and buy-box coverage so OK ingest can persist.
-- See docs/01-architecture/adr/0004-oklahoma-city-ok-region.md
-- =============================================================================
...
-- ── Buy-box: include OK in broad + truck rules ───────────────────────────────

UPDATE tav.buy_box_rules
SET regions = array_append(regions, 'oklahoma_city_ok')
WHERE rule_id IN ('bbr-all-2018-2023-100k', 'bbr-truck-2017-2023-120k')
  AND regions IS NOT NULL
  AND NOT ('oklahoma_city_ok' = ANY(regions));
