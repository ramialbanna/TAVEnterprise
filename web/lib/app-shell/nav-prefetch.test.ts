...
describe("nav-prefetch", () => {
  ...
  it("prefetchNavHref warms Blocked sellers", async () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    prefetchNavHref(client, "/admin/blocked-sellers");
    await vi.waitFor(() => {
      expect(mockedBlocked).toHaveBeenCalled();
    });
  });
